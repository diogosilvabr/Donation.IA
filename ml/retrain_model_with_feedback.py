...
def carregar_e_preprocessar_dados(caminho_dados, feedback_dados=None):
    ...
    if feedback_dados:
        df_feedback = pd.read_csv(feedback_dados, names=[
            'text', 'textoLimpo', 'homophobia', 'obscene', 'insult', 
            'racism', 'misogyny', 'xenophobia', 'homophobiaCorrection', 
            'obsceneCorrection', 'insultCorrection', 'racismCorrection', 
            'misogynyCorrection', 'xenophobiaCorrection'], 
            encoding='latin1', skiprows=1, on_bad_lines='skip')
        
        # Usar as correções no lugar das predições originais
        df_feedback['homophobia'] = df_feedback['homophobiaCorrection']
        df_feedback['obscene'] = df_feedback['obsceneCorrection']
        df_feedback['insult'] = df_feedback['insultCorrection']
        df_feedback['racism'] = df_feedback['racismCorrection']
        df_feedback['misogyny'] = df_feedback['misogynyCorrection']
        df_feedback['xenophobia'] = df_feedback['xenophobiaCorrection']
        
        df_feedback = df_feedback[['text', 'textoLimpo', 'homophobia', 'obscene', 'insult', 
...
def treinarModelo(dados):
    ...
    for coluna in ['homophobia', 'obscene', 'insult', 'racism', 'misogyny', 'xenophobia']:
        ...
        metricas_iniciais[coluna] = classification_report(y_teste, y_teste_pred, output_dict=True)
...
def avaliarModelo(modelos, vetorizador, dados):
    ...
    for coluna in ['homophobia', 'obscene', 'insult', 'racism', 'misogyny', 'xenophobia']:
        ...
        metricas_finais[coluna] = classification_report(y, y_pred, output_dict=True)
